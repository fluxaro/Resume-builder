...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      setPreviewScale(Math.max(Math.min((w - 32) / 794, 1), 0.3));
    });
  ...
  const captureCanvas = async (): Promise<HTMLCanvasElement | null> => {
    ...
    if (!el) { alert('Preview not found. Switch to the Preview tab first.'); return null; }
                ...
                <div className="flex-1 flex gap-1 overflow-x-auto scrollbar-hide">
                  {TEMPLATE_IDS.map(id => (
                    <button key={id} onClick={() => updateDesign({ template: id })}
                      className={`shrink-0 px-2.5 py-1 rounded-lg text-[11px] font-semibold transition whitespace-nowrap ${activeTemplate === id ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800'}`}>
                      {TEMPLATE_NAMES[id]}
                    </button>
                  ))}
                </div>
...
function NavItem({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-medium text-sm ${active ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
